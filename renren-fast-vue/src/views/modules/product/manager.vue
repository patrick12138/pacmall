...
export default {
  ...
  mounted() {
    this.catPathSub = Sub.subscribe("catPath", (msg, val) => {
      this.dataForm.catelogId = val[val.length - 1];
    });
    this.brandIdSub = Sub.subscribe("brandId", (msg, val) => {
      this.dataForm.brandId = val;
    });
  },
  beforeDestroy() {
    Sub.unsubscribe(this.catPathSub);
    Sub.unsubscribe(this.brandIdSub);
  } //生命周期 - 销毁之前
};
</script>
